...
private:
  ...
  void enablePinPulse();
  void write(char);
#define LCD_RS_HARDCODED 13
#define LCD_ENABLE_HARDCOED 12
...
//(int numCols, int numRows, int rs, int enable, int d4, int d5, int d6, int d7)
...
void lib4LCD::sendCommand(uint8_t value)
{
  ...
  digitalWrite(LCD_RS_HARDCODED, LOW);
  //first nibble
...
void lib4LCD::sendData(uint8_t value)
{
  ...
  digitalWrite(LCD_RS_HARDCODED, HIGH); //the only difference with sendCommand routine
